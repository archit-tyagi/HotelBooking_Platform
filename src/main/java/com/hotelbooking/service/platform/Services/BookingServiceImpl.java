...
import com.hotelbooking.service.platform.Repositories.BookingRepository;
import com.hotelbooking.service.platform.Repositories.GuestRepository;
...
import com.hotelbooking.service.platform.Services.Interfaces.BookingService;
import com.hotelbooking.service.platform.Services.Interfaces.CheckoutService;
import com.hotelbooking.service.platform.Strategy.PricingService;
    ...
    private void handleCheckoutSessionCompleted(Event event) {
        ...
        try {
            if (deserializer.getObject().isPresent()) {
                ...
                stripeObject = deserializer.deserializeUnsafe(); //version-mismatch fallback
            }
        } catch (EventDataObjectDeserializationException e) {
            log.error("Could not deserialize Stripe event {} data object", event.getId(), e);
            return;
        }
        ...
        if (sessionId == null || sessionId.isBlank()) {
            log.warn("checkout.session.completed event {} had no usable session id; skipping", event.getId());
            return;
        }
        ...
        if (BookingStatus.CONFIRMED.equals(booking.getBookingStatus())) {
            log.info("Booking ID {} already CONFIRMED; ignoring duplicate event", booking.getId());
            return;
        }
